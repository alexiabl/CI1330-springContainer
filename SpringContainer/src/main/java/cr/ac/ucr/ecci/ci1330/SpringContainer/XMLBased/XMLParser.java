...
public class XMLParser {
    ...
    public HashMap obtainBeanAttributes(Element beanTag){
        ...
        Object instance= xmlBeanFactory.createBeanInstance(beanTag);
        if(instance != null){
            tagsBeanContent.put(beanTag.getAttributeValue("id"), beanTag);
        }
        beanAttributes.put("beanInstance", instance);
    ...
    public Element findBean(String ref, AutowiringMode autowiringMode){
        ...
        while (!found && i < beans.size()) {
            ...
            if (autowiringMode.equals(AutowiringMode.BYNAME) && idValue.equals(ref) || autowiringMode.equals(AutowiringMode.BYTYPE) && className.equals(ref) ) { //realiza la comparación de clases
                return currentBean;
            }
            i++;
        }
